...
def mlp_categorical_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    return logp_all, pi, logp, logp_pi, info, info_phs, d_kl
...
def mlp_gaussian_policy(x, a, hidden_sizes, activation, output_activation, action_space):
    ...
    return mu, pi, logp, logp_pi, info, info_phs, d_kl


"""
Actor-Critics
"""
def mlp_actor_critic(x, a, x_nxt, x_bar, a_hidden_sizes=(64,32), q_hidden_sizes=(64,32), activation=tf.tanh, 
    ...
    with tf.variable_scope('pi'):
        policy_outs = policy(x, a, a_hidden_sizes, activation, output_activation, action_space)
        mu, pi, logp, logp_pi, info, info_phs, d_kl = policy_outs
    with tf.variable_scope('pi', reuse=True):
        mu_nxt, pi_nxt, _, _, _, _, _ = policy(x_nxt, a, a_hidden_sizes, activation, output_activation, action_space)
    with tf.variable_scope('pi', reuse=True):
        mu_bar, pi_bar, _, _, _, _, _ = policy(x_bar, a, a_hidden_sizes, activation, output_activation, action_space)
    with tf.variable_scope('v'):
        v = tf.squeeze(mlp(x, list(q_hidden_sizes)+[1], activation, None), axis=1)
    return mu, pi, mu_nxt, pi_nxt, mu_bar, pi_bar, logp, logp_pi, info, info_phs, d_kl, v
