...
import gym
import time
import rl_smoothness.algs.cores.vpg_core as core
...
def vpg(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0, 
        eps_per_epoch=10, epochs=10, gamma=0.99, pi_lr=3e-4,
        vf_lr=1e-3, train_v_iters=80, lam=0.97, lam_a=0.4, lam_s=2.5,
        logger_kwargs=dict(), save_freq=10, args={'training_dir':'./tmp_train', 'ckpt_dir':'./tmp_train/checkpoints'}):
    """
        ...
        for t in range(local_steps_per_epoch):
            ...
            if terminal or (t==local_steps_per_epoch-1):
                ...
                if ep_counter%test_period == 0:
                    ...
                    test_save(env, get_action, "VPG", deterministic=deterministic)
                    if (args['env_type'] != 'degenerate') and (args['env_type'] != args['test_env_type']):
