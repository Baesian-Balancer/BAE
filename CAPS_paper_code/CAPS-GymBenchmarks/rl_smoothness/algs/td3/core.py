...
def mlp_actor_critic(x, a, x_nxt, x_bar, hidden_sizes=(256,256), activation=tf.nn.relu, 
    ...
    with tf.variable_scope('pi', reuse=True):
        pi_nxt = act_limit * mlp(x_nxt, list(hidden_sizes)+[act_dim], activation, output_activation)
    with tf.variable_scope('pi', reuse=True):
        pi_bar = act_limit * mlp(x_bar, list(hidden_sizes)+[act_dim], activation, output_activation)
