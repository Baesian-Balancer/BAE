...
Actor-Critics
"""
def mlp_actor_critic(x, a, x_nxt, x_bar, hidden_sizes=(64,64), activation=tf.tanh, 
    ...
    with tf.variable_scope('pi', reuse=True):
        mu_nxt, pi_nxt, _, _, _, _, _ = policy(x_nxt, a, hidden_sizes, activation, output_activation, action_space)
    with tf.variable_scope('pi', reuse=True):
        mu_bar, pi_bar, _, _, _, _, _ = policy(x_bar, a, hidden_sizes, activation, output_activation, action_space)
