...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            # leg_action.append(a[0])
            # knee_action.append(a[1])
            change_leg = a[0] - a_first_leg
            change_knee = a[1] - a_first_knee

            a_first_leg = a[0]
            a_first_knee = a[1]

            leg_change.append(change_leg)
            knee_change.append(change_knee)
            timesteps = range(local_steps_per_epoch)
    ...
    plt.figure()
    # plt.hist(leg_action)
    # plt.hist(knee_action)
    plt.plot(timesteps,leg_change)
    plt.plot(timesteps,knee_change)
    plt.show()
