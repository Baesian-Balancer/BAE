...
def sac(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0,
    ...
    def compute_loss_q(data):
        ...
        with torch.no_grad():
            # CAPS loss
            mu1, _ = ac.pi(o, deterministic=True, with_logprob=False)
            mu2, _ = ac.pi(o2, deterministic=True, with_logprob=False)

            if config['lam_s'] > 0:
                mu_bar, _ = self.pi(torch.normal(o, config['std_s']), deterministic=True, with_logprob=False)
        ...
        if config['lam_a'] > 0:
            temporal_smoothness = torch.norm(mu1 - mu2)
            loss_q += config['lam_a'] * temporal_smoothness
        if config['lam_s'] > 0:
            spatial_smoothness = torch.norm(mu - mu_bar)
            loss_q += config['lam_s'] * spatial_smoothness
        if config['lam_o'] > 0:
            state_smoothness = torch.norm(o - o2)
            loss_q += config['lam_o'] * state_smoothness
