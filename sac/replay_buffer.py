...
class ReplayBuffer(object):
    ...
    def __init__(self, obs_shape, action_shape, cfg, device):
        self.capacity = int(cfg.replay_buffer_capacity)
        self.device = device
        self.name = 'uniform'
    ...
    def sample(self, batch_size):
        ...
        return idxs,obses, actions, rewards, next_obses, not_dones, not_dones_no_max,weights
...
class ReplayBufferPER():
  def __init__(self, obs_shape, action_shape, cfg, device):
    ...
    obs_dtype = np.float32 if len(obs_shape) == 1 else np.uint8
    transition_dtype = np.dtype([('state', obs_dtype,obs_shape), ('next_state', obs_dtype,obs_shape), ('action', np.float32,action_shape), ('reward', np.float32), ('not_dones', np.float32),('not_dones_no_max',np.float32)])
    self.blank_trans = (np.zeros(obs_shape,dtype=obs_dtype), np.zeros(obs_shape,dtype=obs_dtype),np.zeros(action_shape,dtype=np.float32), 0.0, 0.0, 0.0)

    self.name = "per"
    self.priority_metric = "td_error"
    ...
    self.transitions = SegmentTree(capacity,transition_dtype,self.blank_trans)  # Store transitions in a wrap-around cyclic buffer within a sum tree for querying priorities

  # Adds state and action at time t, reward and terminal at time t + 1
  def add(self, obs, action, reward, next_obs, done, done_no_max):
    self.transitions.append((obs,next_obs, action, reward, not done,not done_no_max), self.transitions.max)  # Store new transition with maximum priority
  ...
  def _get_samples_from_segments(self, batch_size, p_total):
    ...
    obs  = torch.as_tensor(transitions['state'],device=self.device).float()
    next_obs = torch.as_tensor(transitions['next_state'],device=self.device).float()
    actions = torch.as_tensor(transitions['action'],device=self.device)
    rewards = torch.as_tensor(transitions['reward'],device=self.device).unsqueeze(-1)
    not_dones = torch.as_tensor(transitions['not_dones'],device=self.device).unsqueeze(-1)
    not_dones_no_max = torch.as_tensor(transitions['not_dones_no_max'],device=self.device).unsqueeze(-1)
    ...
    return probs, idxs, tree_idxs, obs,actions,rewards,next_obs,not_dones,not_dones_no_max

  def sample(self, batch_size):
    p_total = self.transitions.total()  # Retrieve sum of all priorities (used to create a normalised probability distribution)
    probs, idxs, tree_idxs, states, actions, returns, next_states, not_dones,not_dones_no_max = self._get_samples_from_segments(batch_size, p_total)  # Get batch of valid samples
    probs = probs / p_total  # Calculate normalised probabilities
    capacity = self.capacity if self.transitions.full else self.transitions.index
    weights = (capacity * probs) ** -self.priority_weight  # Compute importance-sampling weights w
    weights = torch.tensor(weights / weights.max(), dtype=torch.float32, device=self.device)  # Normalise by max importance-sampling weight from batch
    return tree_idxs, states, actions, returns, next_states, not_dones,not_dones_no_max, weights
